...
def apply_rolling_functions(df, col='GR', window=10,
    ...
    for k, v in func.items():
        series = df.loc[:, col].rolling(window=window, center=center, min_periods=1).apply(v, raw=True)
...
def preprocess_a_well(df_well):
    ...
    df_feats_wl_right = apply_rolling_functions(df_well.copy(), window=50, col='GR_medfilt',center=False)
